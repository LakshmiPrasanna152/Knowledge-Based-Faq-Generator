...
function SpinnerBlue() {
  return (
    <span style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
      <svg width="20" height="20" viewBox="0 0 40 40" style={{ animation:"spin 1s linear infinite" }}>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(0deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:1 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(30deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.85 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(60deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.7 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(90deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.55 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(120deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.4 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(150deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.3 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(180deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.2 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(210deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.15 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(240deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.1 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(270deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.08 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(300deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.06 }}/>
        <circle cx="20" cy="20" r="3" fill="#9ca3af" style={{ transform:"rotate(330deg) translate(0,-12px)", transformOrigin:"20px 20px", opacity:0.04 }}/>
      </svg>
    </span>
  );
}
...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      const c = err.code || "";
      if      (c === "auth/email-already-in-use")  setError("This email is already registered. Please sign in.");
      ...
      else if (c === "auth/operation-not-allowed")  setError("Email/password sign-up is disabled. Enable it in Firebase Console → Authentication → Sign-in method.");
      else if (c === "auth/network-request-failed") setError("Network error. Check your connection.");
      else                                           setError("Sign up failed: " + (err.message?.split("(")[0].replace("Firebase:","").trim() || "Please try again."));
    }
    setFormLoad(false);
  };
    ...
    <div style={{ minHeight:"100vh", display:"flex", background:bg }}>
      ...
      <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"40px 24px", position:"relative" }}>
        ...
        <div style={{ width:"100%", maxWidth:440, background:card, border:`1px solid ${cardBorder}`, borderRadius:20, padding:"36px 32px" }}>
          ...
          <h1 style={{ fontSize:22, fontWeight:700, color:text, marginBottom:6 }}>Create your account</h1>
          <p  style={{ fontSize:13, color:muted, marginBottom:24 }}>Start generating AI-powered FAQs for free</p>

          {/* success banner */}
          {success && (
            <div style={{ background: dark?"rgba(34,197,94,0.12)":"#f0fdf4", border:`1px solid ${dark?"#166534":"#bbf7d0"}`, borderRadius:10, padding:"10px 14px", marginBottom:18, display:"flex", alignItems:"center", gap:8 }}>
              <FiCheck color="#22c55e" size={14} />
          ...
          <form onSubmit={handleSignup} noValidate style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            <div style={{ position:"relative" }}>
              <FiUser style={{ position:"absolute", left:13, top:"50%", transform:"translateY(-50%)", color:muted, pointerEvents:"none" }} size={14} />
            ...
            <button type="submit" disabled={formLoad || gLoad}
              ...
              {formLoad
                ...
                : <><span>Create Account</span><FiArrowRight size={14}/></>}
            </button>
          </form>
      ...
      <div className="signup-panel" style={{
        ...
        <div style={{ position:"absolute", bottom:-80, left:-80, width:280, height:280, borderRadius:"50%", background:"rgba(37,99,235,0.12)", filter:"blur(60px)", pointerEvents:"none" }} />
        ...
        <div>
          <h2 style={{ color:"#fff", fontSize:"clamp(1.4rem,2.5vw,2rem)", fontWeight:800, lineHeight:1.3, marginBottom:16 }}>
            ...
            <span style={{ color:"#93c5fd" }}>FAQ workflow</span>
          </h2>
          <p style={{ color:"#93c5fd", fontSize:14, lineHeight:1.7, marginBottom:28 }}>
            Join and start transforming documents into structured AI-generated FAQs.
          </p>
          {["Free to get started","Upload unlimited documents","Export HTML, JSON, TXT","Quiz mode to test knowledge","Tone rewriting & doc comparison"].map(f => (
            <div key={f} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
              <div style={{ width:18, height:18, borderRadius:"50%", border:"1px solid rgba(74,222,128,0.4)", background:"rgba(34,197,94,0.15)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
      ...
      <style>{`
        ...
        @media (max-width: 768px) { .signup-panel { display: none !important; } .mobile-logo { display: flex !important; } }
